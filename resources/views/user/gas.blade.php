        ...
        <div class="w-full max-w-6xl rounded bg-white shadow-xl p-10 lg:p-20 mx-auto text-gray-800 relative md:text-left">
            <div class="md:flex items-center -mx-10">
                <div class="w-full md:w-1/2 px-10 mb-10 md:mb-0">
                    <div class="relative">
                        <img src="images/logo/gasicona.png" class="w-full relative z-10" alt="">
                    </div>
                </div>
                <div class="w-full md:w-1/2 px-10">
                    <div class="mb-10">
                        <h1 class="font-bold uppercase text-2xl mb-5">Gas</h1>
                        <p class="text-sm">€ {{ $item->prezzo }} /mese*</p>
                        <p class="text-sm">{{ $item->mcgas }} smc/mese</p>
                    </div>
                    <div>
                        <div class="block align-bottom mr-5">
                            <span class="text-2xl leading-none align-baseline">€ {{ $item->prezzoKw }}/smc*</span>
                        </div>
                        <br>
                        <div class="block align-bottom">
                            <form action="{{ route('attiva') }}" method="POST">
                                @csrf
                                <input type="hidden" name="id" value="{{ $item->id }}">
                                <button class="bg-blue-500 opacity-75 hover:opacity-100 text-white rounded-full px-10 py-2 font-semibold"><i class="mdi mdi-cart -ml-2 mr-2"></i> Attiva</button>
                            </form>
                        </div>
                    </div>
                </div>
