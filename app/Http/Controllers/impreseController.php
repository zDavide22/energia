...
class impreseController extends Controller
{
    public function luce()
    {
        ...
        return view('imprese.luceimp',['contratti'=>$contratto]);
    }
    ...
    public function attiva(Request $req)
    {
        ...
        return view('imprese.attivaimp',['contratto'=>$contratto,'idCliente'=>$id]);
    }
    ...
    public function gas()
    {
        ...
        return view('imprese.gasimp',['contratti'=>$contratto]);
    }
    ...
    public function removeContratto(Request $req)
    {
        ...
        return view('imprese.contrattiImp',['data'=>$data,'letture'=>$letture,'date'=>$date,'lettureG'=>$lettureG,'dateG'=>$dateG]);
    }
    public function bolletta(Request $req)
    {
        ...
        return view('imprese.bollettaImp',['data'=>$data]);
    }
    ...
    public function bollettaPag(Request $req)
    {
        ...
        return view('imprese.pagamentoImp',['data'=>$data,'cliente'=>$cliente,'contatore'=>$contatore,'contratto'=>$contratto]);
    }
    ...
    public function vedi(Request $req)
    {
        ...
        $contatore= contatoreModel::where('id','=',$idContatore)->first();
        $contratto= contrattoModel::where('idC','=',$idContatore)->first();
        return view('imprese.vediImp',['data'=>$data,'cliente'=>$cliente,'contatore'=>$contatore,'contratto'=>$contratto]);
    }
        ...
        if(is_null($controllo))
        {
            ...
            if(!is_null($cliente->tipo)){
                ...
                if ($cliente->tipo=='Luce') {
                    ...
                    $insert->POD = generateRandomStringI($cliente->tipo);
                    $insert->Modello = 'Modello '.$req->idContratto;
                    $insert->Tipo = $cliente->tipo;
                }else {
                    $insert = new contatoreModel();
                    $insert->PDR = generateRandomStringI($cliente->tipo);;
                    $insert->Modello = 'Modello '.$req->idContratto;
                    $insert->Tipo = $cliente->tipo;
                }
                $insert->save();
            ...
            if ($cliente->tipo=='Luce') {
                return redirect()->route('luceI')->with('error', 'Esiste già un servizio attivo associato a quel indirizzo');
            }else {
                return redirect()->route('gasI')->with('error', 'Esiste già un servizio attivo associato a quel indirizzo');
            }
        }
        $data=contrattoModel::where([
    ...
    public function letturaGrafico(Request $req)
    {
        ...
        if (!empty($req->idC)) {
            ...
            if (count($controllo)==0&& $req->tipo=='Gas') {
                ...
                if (!empty($req->mcgas)) {
                    return redirect()->route('ContrattiI')->with('error', 'Limite di letture giornaliere raggiunto');
                }
            }
    ...
    public function scarica(Request $req)
    {
        ...
        view()->share('dataI',$dataI);
        	// pass view file
            $pdf = PDF::loadView('pdfImp');
            // download pdf
            return $pdf->download('bolletta '.$data->data.'.pdf');
    }
}

function generateRandomStringI($tipo) {
